...
ServiceUpperName ?= "Role"
ServiceLowerName ?= "role"
...
# 复制proto文件
	...
	@cp ./api/role/v1/role.proto ./api/$(ServiceLowerName)/v1/$(ServiceLowerName).proto

# 删除旧文件
	@rm -rf ./api/role
# 生成proto客户端文件
	@ kratos proto client ./api/$(ServiceLowerName)/v1/$(ServiceLowerName).proto
# 替换 Role $(ServiceUpperName), role 为$(ServiceLowerName)并迁移文件
	@grep  -rl Role ./ | xargs sed -i 's/Role/$(ServiceUpperName)/g'
	@grep  -rl role ./ | xargs sed -i 's/role/$(ServiceLowerName)/g'

# 迁移文件
	@ mv ./internal/service/role.go ./internal/service/$(ServiceLowerName).go
	@ mv ./internal/biz/role.go ./internal/biz/$(ServiceLowerName).go
	@ mv ./internal/data/role.go ./internal/data/$(ServiceLowerName).go
	@ mv ./internal/domain/role.go ./internal/domain/$(ServiceLowerName).go
# 拉取引用包
	go mod tidy
